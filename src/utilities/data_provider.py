...
class DataProvider:
    ...
    def get_tooltips(widget_tooltip: str) -> dict:
        with open(str(pathlib.Path(__file__).parent.parent.joinpath("config", DataProvider.get_language_code(), "tooltips.json")), "r", encoding="utf-8") as file:
    ...
    @staticmethod
    def get_errors() -> dict:
        with open(str(pathlib.Path(__file__).parent.parent.joinpath("config", DataProvider.get_language_code(), "errors.json")), "r", encoding="utf-8") as file:
            errors = json.load(file)
            return errors

    @staticmethod
    def get_ui_text(widget_tooltip: str) -> dict:
        with open(str(pathlib.Path(__file__).parent.parent.joinpath("config", DataProvider.get_language_code(), "ui_text.json")), "r", encoding="utf-8") as file:
